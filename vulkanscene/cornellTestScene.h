...
namespace GLSLPT
{
	void loadCornellTestScene(Scene* scene)
	{
		
		scene->addCamera(glm::vec3(0.25f, 0.2f, -0.150f), glm::vec3(0.0f, 0.0f, 0), 39.0f);
		...
		light.u = glm::vec3(.34299999f, .54779997f, .32700010f) - light.position;
		light.v = glm::vec3(.21300001f, .54779997f, .22700010f) - light.position;
		light.area =  glm::length(glm::cross(glm::vec3(light.u.x, light.u.y, light.u.z), glm::vec3(light.v.x, light.v.y, light.v.z)));
