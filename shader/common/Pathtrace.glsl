...
//-----------------------------------------------------------------------
{
	vec4 n1 = normals[state.triID.x].xyzw;
	vec4 n2 = normals[state.triID.y].xyzw;
	vec4 n3 = normals[state.triID.z].xyzw;
...
//-----------------------------------------------------------------------
{
	...
	if (ubo.numOfLights > 0)
	{
		LightSampleRec lightSampleRec;
		Light light;

		//Pick a light to sample
		int index = int(rand() * float(ubo.numOfLights));

		// Fetch light Data
		vec3 p = lights[index].position;
		vec3 e = lights[index].emission;
		vec3 u = lights[index].u;
		vec3 v = lights[index].v;
		vec3 rad = lights[index].radiusAreaType;
